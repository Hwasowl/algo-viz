...
export const sortingTopic = {
  ...
  plainDef: '여러 개의 숫자(또는 데이터)를 작은 것부터 큰 것 순서로 줄 세우는 것.',
  analogy: '키 순서대로 학생들을 줄 세우는 것과 같다. 두 명씩 키를 비교해 자리를 바꾸다 보면 결국 전체가 정렬된다.',
  why: '정렬해 두면 "가장 큰 값 찾기", "이진탐색", "중복 제거" 같은 작업이 훨씬 빨라진다. 거의 모든 알고리즘의 밑바탕.',
  terms: [
    { word: '비교 기반 정렬', desc: '두 값의 크기를 비교(>, <)해서 순서를 정하는 정렬. 버블·선택·삽입·병합·퀵이 모두 여기 속한다.' },
    { word: 'TimSort', desc: '병합 정렬 + 삽입 정렬을 섞은 안정 정렬. 거의 정렬된 데이터에서 매우 빠르다. Java가 객체 정렬에 쓴다.' },
    { word: '안정 정렬', desc: '값이 같은 원소들의 원래 순서가 정렬 후에도 유지되는 정렬. 병합 정렬은 안정, 퀵 정렬은 불안정.' },
    { word: 'Comparator', desc: '"무엇을 기준으로 정렬할지"를 정의하는 Java 객체. 예: 나이순, 이름순처럼 정렬 기준을 직접 지정.' },
  ],
  coteTip:
    '코테에서는 정렬 알고리즘을 직접 구현할 일은 드물고, Arrays.sort / Collections.sort를 쓰되 "무엇을 기준으로 정렬할지"(Comparator)를 정하는 게 핵심이다. ' +
    '단, O(n²) 정렬(버블·선택·삽입)은 n이 10만을 넘으면 시간 초과가 나므로 O(n log n) 표준 정렬을 써야 한다는 감각이 중요하다.',
  concept:
